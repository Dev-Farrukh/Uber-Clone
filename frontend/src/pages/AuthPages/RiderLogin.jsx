  ...
  return (
    <div className="flex flex-col justify-around min-h-dvh bg-gray-100 font-poppins lg:justify-center lg:px-6">
      <main className="flex flex-col gap-10 w-full max-w-md mx-auto lg:rounded-2xl lg:bg-white lg:p-8 lg:h-110 lg:shadow-sm">
        <div><h1 className="pl-5 text-3xl lg:pl-0">Sign In As Rider</h1></div>
        <form onSubmit={submitHandler} className="flex flex-col gap-5 px-5 lg:px-0">
          ...
          <div className="text-center text-sm">Don't have an account ? <Link to="/rider-register" className="text-[#EDAF10] font-semibold hover:underline">Signup</Link></div>
        </form>
      </main>

      <Link to="/login" className="p-3 bg-[#EDAF10] rounded-sm text-white hover:bg-[#dea922] mx-5 text-center max-w-md md:mx-auto  md:w-full lg:mt-6 lg:mx-auto">Sign in as User </Link>
    </div>
  )
}
